...
from src.services.vpn_config import (
    ...
    SUBSCRIPTION_REMARK,
    build_expired_fallback_share_links,
    build_expired_limited_share_links,
    build_expired_limited_subscription_payload,
    build_expired_telegram_xray_config,
    build_happ_ru_direct_routing_link,
    build_happ_telegram_only_routing_link,
...
from src.services.xray_sync import _config_points_to_panel, _config_points_to_yandex_entry
router = APIRouter()
...
def _build_active_headers(subscription, settings: Settings) -> dict[str, str]:
    ...
    return {
        ...
        "x-hwid-active": "true",
        "routing": build_happ_ru_direct_routing_link(activate=True),
...
def _build_inactive_headers(
    ...
    *,
    ...
    # HWID on expired/suspended feeds blocks Happ refresh for some clients — disable.
    if reason == InactiveReason.EXPIRED:
        ...
        if telegram_only:
            headers["routing"] = build_happ_telegram_only_routing_link(activate=True)
    else:
...
async def _pick_announce_config(session: AsyncSession, settings: Settings) -> tuple[str, str] | None:
    """Pick a managed (Yandex/Finland) config template for expired Telegram-only access."""
    try:
        configs = await ConfigCredentialService(session, settings).list_active_configs()
    except Exception:
        logging.getLogger(__name__).exception("Failed to list configs for expired announce")
        return None
    panel: list = []
    yandex: list = []
    for config in configs:
        name = config.name or ""
        if name in FINLAND_NAME_ALIASES or _config_points_to_panel(config):
            panel.append(config)
        elif name in LTE_TUNNEL_NAME_ALIASES or _config_points_to_yandex_entry(config):
            yandex.append(config)
    chosen = panel or yandex
    if not chosen:
        return None
    config = chosen[0]
    template = (config.config_template or "").strip()
    if not template:
        return None
    return config.config_type.value, template


def _expired_share_links(announce: tuple[str, str] | None) -> list[str]:
    """Build expired instructional links; always falls back to panel announce."""
    if announce:
        config_type, template = announce
        try:
            return build_expired_limited_share_links(config_type, template)
        except Exception:
            logging.getLogger(__name__).exception(
                "Failed building expired links from template — using fallback"
            )
    return build_expired_fallback_share_links()


async def _build_inactive_response(
    subscription,
    settings: Settings,
    reason: InactiveReason,
    format: str,
    ...
    announce = None
    if reason == InactiveReason.EXPIRED:
        announce = await _pick_announce_config(session, settings)

    # Always serve telegram-only instructional feed for expired (fallback if needed).
    telegram_only = reason == InactiveReason.EXPIRED
    headers = _build_inactive_headers(
        subscription, settings, reason, telegram_only=telegram_only
    )
    remark = _inactive_remark(reason)

    if telegram_only:
        links = _expired_share_links(announce)
        if format == "json":
            if announce:
                try:
                    return JSONResponse(
                        content=build_expired_telegram_xray_config(announce[1]),
                        headers=headers,
                    )
                except Exception:
                    logging.getLogger(__name__).exception(
                        "Expired JSON profile failed — returning message payload"
                    )
            return JSONResponse(
                content={
                    "remarks": "❌ Подписка закончилась",
        ...
        # default / profile / xjson / full → base64 share lines
        return PlainTextResponse(
            content=build_multi_share_links_payload(links),
            media_type="text/plain; charset=utf-8",
            headers=headers,
        )
...
async def _resolve_inactive_reason(
    ...
    # Expired first — never block Happ refresh on Xray sync / device-limit checks.
    ...
    if subscription.expires_at <= utcnow():
        await SubscriptionService(session, settings).expire_subscription(
            subscription, sync_xray=False
        )
        return InactiveReason.EXPIRED
...
async def subscription_feed(
    ...
    if inactive_reason:
        return await _build_inactive_response(
            subscription, settings, inactive_reason, format, session
        )
    ...
    # Happ: routing profile в теле подписки (WB/Ozon → direct)
    routing_link = build_happ_ru_direct_routing_link(activate=True)
    device_links = [routing_link, *device_links]
    if format == "link":
        return PlainTextResponse(
...
async def subscription_raw_json(
    ...
    if inactive_reason:
        return await _build_inactive_response(
            subscription, settings, inactive_reason, "json", session
        )
    config_store = VpnConfigStore(session)
    profile_config = await config_store.resolve_profile_config(subscription.config_id)
