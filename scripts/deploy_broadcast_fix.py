"""Deploy telegram broadcast fix; restart bot only."""
...
def main() -> int:
    ...
    out, err, _ = run(
        ...
        "from src.bot.keyboards.inline import admin_broadcast_confirm_keyboard, broadcast_confirm_keyboard; "
        "kb=admin_broadcast_confirm_keyboard('all'); "
        "kb2=broadcast_confirm_keyboard(); "
        "print('import_ok', len(kb.inline_keyboard), len(kb2.inline_keyboard))\"",
    )
    print(out)
    if err:
        print(err)
    ...
    out, err, _ = run(
        ...
        "journalctl -u qooq-bot --since '30 sec ago' --no-pager | tail -n 20",
    )
    print(out)
    if err:
        print(err)
